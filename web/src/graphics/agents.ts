...
export type AgentContainer = Container;

type AgentUpdateData = { id: string; pos: string; data: number; label?: string };

export function setUpAgents(app: App): AgentContainer {
  const agentsContainer = new Container();
  app.centered.addChild(agentsContainer);
...
export function agentsFromData(
  ...
  agentsLayout(byPort, network, radius, ({ id }, x, y) => {
    const agentObj = new Agent(x, y, id, id);
    agentsContainer.addChild(agentObj);
  });
}
